...
{	
	...
	function add()
	{
		$max_order_qty=$this->db->get_where('phppos_location_items', array('item_id'=>$this->input->post("item")))->row()->max_order_qty;
		$current_qty=$this->db->get_where('phppos_location_items', array('item_id'=>$this->input->post("item")))->row()->quantity;
		$total_qty=to_quantity($current_qty)+1;
		if($total_qty>$max_order_qty){
			$data['error']='Current quantity must not be greater than the maximum order quantity.';
			...
			if($this->input->post("name"))
			{
				...
				if ($item = $this->cart->get_item($line))
				{
					try
					{
						...
						//  create joel
						$max_order_qty=$this->db->get_where('phppos_location_items', array('item_id'=>$item->item_id))->row()->max_order_qty;
						$current_qty=$this->db->get_where('phppos_location_items', array('item_id'=>$item->item_id))->row()->quantity;
						$current_qty=to_quantity($current_qty);
						$max_order_qty=to_quantity($max_order_qty);
						if($this->cart->get_mode() == 'receive' && $variable=='quantity' && $$variable>$max_order_qty){						
							$data['error']='Current quantity must not be greater than the maximum order quantity.';
							return $this->_reload($data);
						}
						$total_qty=$$variable+$current_qty;
						if($this->cart->get_mode() == 'receive' && $variable=='quantity'&& $total_qty>$max_order_qty){						
							$data['error']='Current quantity must not be greater than the maximum order quantity.';
							return $this->_reload($data);
						}
						if($variable=='max_order_qty'){
							if($this->cart->get_mode() == 'receive' && $$variable>$current_qty){		
								echo "true";
								$data['error']='Current quantity must not be greater than the maximum order quantity.';
								return $this->_reload($data);
							}
							...
							$this->db->update('phppos_location_items', array("max_order_qty"=>$this->input->post("value")), array("item_id"=>$item->item_id,"location_id"=>$location_id));
							return $this->_reload($data);
						}
						//  end joel
