						...
						else
						{
						 ...
						 foreach(array_reverse($cart_items, true) as $line=>$item) { 
							...
							<tr class="register-item-details">
								...
								<input type="hidden" id="max_order_qty_<?php echo $line;?>"  value="<?php echo to_quantity($mode=="transfer" ? abs($item->max_order_qty) : $item->max_order_qty); ?>">
									  ...
									  <dd  class="">
										...
										{ 
											...
											else
											{
												...
												$max_order_qty = $this->Item_variation_location->get_max_order_qty_data($item->item_id);
											?>
											...
											<?php if ($this->Employee->has_module_action_permission('sales', 'edit_sale_price', $this->Employee->get_logged_in_employee_info()->person_id)) {	?>
												<dt>maximum Order Quantity</dt>
												<dd>													
													<a href="#" id="max_order_qty_<?php echo $line;?>" class="xeditable" data-type="text" data-pk="1" data-name="max_order_qty" data-value="<?php echo to_quantity($max_order_qty) ?>" data-url="<?php echo site_url('receivings/edit_item/'.$line); ?>"><?php echo to_quantity($max_order_qty) ?></a>
												</dd>											
												...
												<dd>
													<?php 
														if($item->selling_price){
															$selling_price=to_currency_no_money($item->selling_price, 10);
														}
													?>
													<a href="#" id="selling_price_<?php echo $line;?>" class="xeditable" data-type="text" data-pk="1" data-name="selling_price" data-value="<?php echo $selling_price ?>"><?php echo $selling_price ?></a>
												</dd>
												<!--  end Joel-->
...
{	
	...
	$("#change_cart_date").datetimepicker({
		minDate: moment().add(-30, 'days'),  // Current day
   		maxDate: moment()
	});
